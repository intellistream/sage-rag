...
class EmbeddingModel:
    def __init__(self, method: str = "default", model: str = "BAAI/bge-m3") -> None:
        if get_embedding_model is None:
            raise ImportError(
                "sagellm.embedding is required for retriever embedding. "
                "Please install isagellm in this environment."
            )
        self._embedder = get_embedding_model(method, model=model)
...
class ChromaRetriever(MapOperator):
    ...
    def _init_embedding_model(self):
        """初始化 embedding 模型（统一通过 sagellm 服务）。"""
        embedding_method = self.embedding_config.get("method", "default")
        model = self.embedding_config.get("model", "BAAI/bge-m3")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise ImportError("Embedding model dependencies not available")  # noqa: B904

            self.embedding_model = BGEM3EmbeddingFunction(use_fp16=False, device="cpu")

        except ImportError as e:
            self.logger.error(f"Failed to initialize sparse embedding function: {e}")
            raise ImportError("Embedding model dependencies not available")  # noqa: B904
...
class Wiki18FAISSRetriever(MapOperator):
    ...
    def _init_bge_m3_model(self):
        """初始化嵌入模型（通过 sagellm embedding 服务）。"""
        try:
            embedding_method = self.embedding_config.get("method", "sagellm")
            model_path = self.embedding_config.get("model", "BAAI/bge-large-en-v1.5")

            self.embedding_model = EmbeddingModel(method=embedding_method, model=model_path)
            self.logger.info(f"嵌入模型初始化成功: method={embedding_method}, model={model_path}")
        except Exception as e:
    ...
    def _init_faiss_index(self):
        ...
        try:
            ...
            if os.path.exists(index_path) and os.path.exists(documents_path):
                ...
                # 创建空索引和文档列表作为占位符
                dimension = int(self.embedding_model.get_dim() or 1024)
    ...
    def build_index_from_wiki18(self, wiki18_data_path: str, save_path: str | None = None):
        ...
        try:
            ...
            # 提取文档文本并编码
            doc_texts = [doc.get("text") or doc.get("contents", "") for doc in documents]

            # 批量编码所有文档
            self.logger.info("开始编码文档...")
            try:
                embeddings = self.embedding_model.embed_batch(doc_texts)
            except NotImplementedError:
                embeddings = [self.embedding_model.embed(text) for text in doc_texts]
            doc_vectors = np.array(embeddings, dtype=np.float32)
